...
with open(R"ch3\UK_text.txt", "r", encoding="utf-8") as UK_text:
    ...
    for key, text in result_list_27.items():
        ...
        pattern = "\:\{\{.*?\}\}"
        # Royal coats of arms:{{#tag:ref |The coat of arms on the left is used in England, Northern Ireland, and Wales; the version on the right is used in Scotland|group=note}}\
        text = re.sub(pattern, "", text)

        # result_list_28[key] = text

        print("28 " + result_list_28[key])
